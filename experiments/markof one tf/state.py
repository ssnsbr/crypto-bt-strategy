...
class TrendStateDetector:
    def __init__(self, *mas):
        self.mas = [ma for ma in mas if ma is not None]
        self.trend = TrendState.UNDECIDED

    def detect(self):
        # Check if all MAs are aligned
        # [3,2,1]
        if all(self.mas[i] > self.mas[i + 1] for i in range(len(self.mas) - 1)):
            return TrendState.TRENDING_DOWN
        elif all(self.mas[i] < self.mas[i + 1] for i in range(len(self.mas) - 1)):
...
class MarketState():
    def __init__(self, market):
        self.market = market
        self.lowerTrendState = TrendState.UNDECIDED
        self.higherTrendState = TrendState.UNDECIDED

    def update_trend_state(self):
        self.lowerTrendState = TrendStateDetector([self.market.ma_1, self.market.ma_2]).detect()
        self.higherTrendState = TrendStateDetector([self.market.ma_3, self.market.ma_4]).detect()
...
class StateDetector:
    ...
    def __init__(self, ):
        ...
        self.high_trend_state = HighTrendState.UNDECIDED,
        self.low_trend_state = LowTrendState.UNDECIDED
