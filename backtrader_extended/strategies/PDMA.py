...
class PriceDistanceFromMA(bt.Indicator):
    """Calculate current price distance from MAs"""
    ...
    def next(self):
        ...
        # Calculate current price distance from each MA
        pd_values = []
        z_values = []

        for ma in self.mas:
            pd = (close - ma[0]) / ma[0] * 100
            pd_values.append(pd)
        ...
        if self.p.useZ:
            use_c = sum(pd * z for pd, z in zip(pd_values, z_values)) / len(pd_values)
        else:
            use_c = sum(pd_values) / len(pd_values)

        # Store for EMA calculation
        if not hasattr(self, 'pd_values'):
            self.pd_values = []
        self.pd_values.append(use_c)

        # Smooth
        ma_pd_smooth = self._calculate_ema(self.pd_values, self.p.smooth)

        # Normalize
        if len(self.pd_values) >= self.p.normalize_len:
            mean = np.mean(self.pd_values[-self.p.normalize_len:])
            std = np.std(self.pd_values[-self.p.normalize_len:])
            self.lines.pd_norm[0] = (ma_pd_smooth - mean) / std if std != 0 else 0
        else:
            self.lines.pd_norm[0] = 0

        # Calculate signal
        if not hasattr(self, 'pd_norm_values'):
            self.pd_norm_values = []
        self.pd_norm_values.append(self.lines.pd_norm[0])
        self.lines.pd_signal[0] = self._calculate_ema(self.pd_norm_values, self.p.signal_len)
