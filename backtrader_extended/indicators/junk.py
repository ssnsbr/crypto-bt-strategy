...
# ==== Pivot Struct ====
class Pivot:
    def __init__(self, pivot_type: PIVOTTYPE, direction: DIRECTION, trend: TREND, price):
    ...
    def __repr__(self):
        return f"{self.pivot_type.name}@{self.price:.2f}"


 class FindTrendByBounce:  def __init__(self):  # list of ('HH', price), ('HL', price), etc.
 self.last_processed_bounce = None self.current_undetermined_price = None def _not_new(self,newb): return self.last_processed_bounce["type"] == newb["type"] and self.last_processed_bounce["start"] == newb["start"] and self.last_processed_bounce["end"] == newb["end"] def update(self, bounce_state): """ Update the structure with the latest bounce info. Args: bounce_state: dict from BounceDetector.get_state() """ bounces = bounce_state.get("bounce_list", []) if not bounces: return if len(bounces) == 1: b = bounces[0] self.last_processed_bounce = b if b["type"] == "up": p1=Pivot(pivot_type=PIVOTTYPE.START, direction=DIRECTION.WITH_TREND, trend=TREND.UP, price=b["start"] ) self.structure.append(p1) p2=Pivot(pivot_type=PIVOTTYPE.HH, direction=DIRECTION.WITH_TREND, trend=TREND.UP, price=b["end"] ) self.structure.append(p2) else: p1=Pivot(pivot_type=PIVOTTYPE.START, direction=DIRECTION.WITH_TREND, trend=TREND.DOWN, price=b["start"] ) self.structure.append(p1) p2=Pivot(pivot_type=PIVOTTYPE.LL, direction=DIRECTION.WITH_TREND, trend=TREND.DOWN, price=b["end"] ) self.structure.append(p2) self.current_undetermined_price = b["end"] return if self._not_new(bounces[-1]): return if self.current_undetermined_price != bounces[-1]["start"]: print("Warning! This should NOT happen.","End is not START!") previous_pivot = self.structure[-1] previous_previous_pivot = self.structure[-2] bounce = bounces[-1] self.current_undetermined_price = bounce["end"] # Determine New Pivot According to Bounce # / + / -> Warning! # / if previous_pivot.pivot_type = PIVOTTYPE.HH and bounce["type"] == "up" or previous_pivot.pivot_type = PIVOTTYPE.LL and bounce["type"] == "down" or previous_pivot.pivot_type = PIVOTTYPE.HL and bounce["type"] == "down" or previous_pivot.pivot_type = PIVOTTYPE.LH and bounce["type"] == "up" : print("Warning! This should NOT happen.") if previous_pivot.PIVOTTYPE = PIVOTTYPE.LL : if previous_previous_pivot.price =< bounce["end"]: # A break out Happened! p=Pivot(pivot_type = PIVOTTYPE.HH, direction = DIRECTION.WITH_TREND, trend=TREND.UP, price=b["end"] ) else : # Normal correction p=Pivot(pivot_type = PIVOTTYPE.LH, direction = DIRECTION.CORRECTION, trend=TREND.DOWN, price=b["end"] ) self.structure.append(p) return if previous_pivot.PIVOTTYPE = PIVOTTYPE.LH : if previous_previous_pivot.price =< bounce["end"]: # Normal Trend p=Pivot(pivot_type = PIVOTTYPE.LL, direction = DIRECTION.WITH_TREND, trend=TREND.DOWN, price=b["end"] ) else : # Werid correction p=Pivot(pivot_type = PIVOTTYPE.HL, direction = DIRECTION.CORRECTION, trend=TREND.UP, price=b["end"] ) self.structure.append(p) return if previous_pivot.PIVOTTYPE = PIVOTTYPE.HH : if previous_previous_pivot.price =< bounce["end"]: # A break out Happened! p=Pivot(pivot_type = PIVOTTYPE.LL, direction = DIRECTION.WITH_TREND, trend=TREND.DOWN, price=b["end"] ) else : # Normal correction p=Pivot(pivot_type = PIVOTTYPE.HL, direction = DIRECTION.CORRECTION, trend=TREND.UP, price=b["end"] ) self.structure.append(p) return if previous_pivot.PIVOTTYPE = PIVOTTYPE.HL : if previous_previous_pivot.price => bounce["end"]: # Normal Trend p=Pivot(pivot_type = PIVOTTYPE.HH, direction = DIRECTION.WITH_TREND, trend=TREND.UP, price=b["end"] ) else : # Werid correction p=Pivot(pivot_type = PIVOTTYPE.LH, direction = DIRECTION.CORRECTION, trend=TREND.DOWN, price=b["end"] ) self.structure.append(p) return def get_trend(self): return self.trend def get_structure(self): return self.structure
# ==== FindTrendByBounce ====
class FindTrendByBounce:
  """ Tracks swing structure (HH, HL, LH, LL) using bounce list from BounceDetector.
  Determines current trend direction (up, down, or neutral). """
    def __init__(self):
        self.structure = []
        self.last_processed = None

    def update(self, bounce_state):
        bounces = bounce_state.get("bounce_list", [])
        if not bounces:
            return

        if self.last_processed == len(bounces):
            return  # already handled latest bounce

        b = bounces[-1]
        if len(self.structure) == 0:
            # initialize first pivot pair
            t = TREND.UP if b["type"] == "up" else TREND.DOWN
            p1 = Pivot(PIVOTTYPE.START, DIRECTION.WITH_TREND, t, b["start"])
            p2 = Pivot(PIVOTTYPE.HH if t == TREND.UP else PIVOTTYPE.LL,
                       DIRECTION.WITH_TREND, t, b["end"])
            self.structure += [p1, p2]
        else:
            prev = self.structure[-1]
            if b["type"] == "up":
                if b["end"] > prev.price:
                    p = Pivot(PIVOTTYPE.HH, DIRECTION.WITH_TREND, TREND.UP, b["end"])
                else:
                    p = Pivot(PIVOTTYPE.LH, DIRECTION.CORRECTION, TREND.DOWN, b["end"])
            else:  # down bounce
                if b["end"] < prev.price:
                    p = Pivot(PIVOTTYPE.LL, DIRECTION.WITH_TREND, TREND.DOWN, b["end"])
                else:
                    p = Pivot(PIVOTTYPE.HL, DIRECTION.CORRECTION, TREND.UP, b["end"])
            self.structure.append(p)

        self.last_processed = len(bounces)
...
class Pivot:
    def __init__(self, pivot_type, direction, trend, price):
        self.pivot_type = pivot_type
        self.direction = direction
        self.trend = trend
        self.price = price
...
class FindTrendByBounce:
    ...
    def __init__(self):
        ...
        self.structure = []  # list of ('HH', price), ('HL', price), etc.
        self.last_processed_bounce = None
        self.current_undetermined_price = None

    def _not_new(self,newb):
        return self.last_processed_bounce["type"] == newb["type"] and
        self.last_processed_bounce["start"] == newb["start"] and
        self.last_processed_bounce["end"] == newb["end"]

    def update(self, bounce_state):
        """
        Update the structure with the latest bounce info.
        Args:
            bounce_state: dict from BounceDetector.get_state()
        """
        bounces = bounce_state.get("bounce_list", [])
        if not bounces:
            return
        ...
        if len(bounces) == 1:
            ...
            if  b["type"] == "up":
                p1=Pivot(pivot_type=PIVOTTYPE.START,
                          direction=DIRECTION.WITH_TREND,
                          trend=TREND.UP,
                          price=b["start"]
                        )
                self.structure.append(p1)
                p2=Pivot(pivot_type=PIVOTTYPE.HH,
                          direction=DIRECTION.WITH_TREND,
                          trend=TREND.UP,
                          price=b["end"]
                        )
                self.structure.append(p2)
            else:
                p1=Pivot(pivot_type=PIVOTTYPE.START,
                          direction=DIRECTION.WITH_TREND,
                          trend=TREND.DOWN,
                ...
                self.structure.append(p1)
                p2=Pivot(pivot_type=PIVOTTYPE.LL,
                          direction=DIRECTION.WITH_TREND,
                          trend=TREND.DOWN,
                          price=b["end"]
                        )
        ...
        if self.current_undetermined_price != bounces[-1]["start"]:
            print("Warning! This should NOT happen.","End is not START!")

        previous_pivot = self.structure[-1]
        previous_previous_pivot = self.structure[-2]
        bounce = bounces[-1]
        self.current_undetermined_price = bounce["end"]

        # Determine New Pivot According to Bounce
        #  /  +   /   -> Warning!
        # /
        if previous_pivot.pivot_type = PIVOTTYPE.HH and bounce["type"] == "up" or
            previous_pivot.pivot_type = PIVOTTYPE.LL and bounce["type"] == "down" or
             previous_pivot.pivot_type = PIVOTTYPE.HL and bounce["type"] == "down" or
               previous_pivot.pivot_type = PIVOTTYPE.LH and bounce["type"] == "up"  :
            print("Warning! This should NOT happen.")

        if previous_pivot.PIVOTTYPE = PIVOTTYPE.LL  :
            if previous_previous_pivot.price =< bounce["end"]:
                # A break out Happened!
                p=Pivot(pivot_type = PIVOTTYPE.HH,
                          direction = DIRECTION.WITH_TREND,
                          trend=TREND.UP,
                          price=b["end"]
                        )
                ...
                # Normal correction
                p=Pivot(pivot_type = PIVOTTYPE.LH,
        ...
        if previous_pivot.PIVOTTYPE = PIVOTTYPE.LH  :
            if previous_previous_pivot.price =< bounce["end"]:
                # Normal Trend
                p=Pivot(pivot_type = PIVOTTYPE.LL,
                          direction = DIRECTION.WITH_TREND,
                          trend=TREND.DOWN,
                          price=b["end"]
                        )
                ...
                # Werid correction
                p=Pivot(pivot_type = PIVOTTYPE.HL,
                          direction = DIRECTION.CORRECTION,
                          trend=TREND.UP,
                          price=b["end"]
                        )
        ...
        if previous_pivot.PIVOTTYPE = PIVOTTYPE.HH  :
            if previous_previous_pivot.price =< bounce["end"]:
                # A break out Happened!
                p=Pivot(pivot_type = PIVOTTYPE.LL,
                          direction = DIRECTION.WITH_TREND,
        ...
        if previous_pivot.PIVOTTYPE = PIVOTTYPE.HL  :
            if previous_previous_pivot.price => bounce["end"]:
                # Normal Trend
                p=Pivot(pivot_type = PIVOTTYPE.HH,
                          direction = DIRECTION.WITH_TREND,
...
class BounceDetector(bt.Indicator):
    # lines = ('state', 'trend', 'last_h', 'last_l')
    params = dict(
        bounce_threshold=0.01,   # 1% swing threshold
    )
    ...
    def next(self):
        price = self.data[0]
        state =  self.bd.detect_bounce(price, up_bounce_threshold=1.1, down_bounce_threshold=0.9)
        self.ft(state)
...
class TestStrategy(BaseCryptoTradingStrategy):
    ...
    def __init__(self):
        super().__init__()
        self.bd = BounceDetector(self.data, bounce_threshold=0.001)

        self.results=[]

    def next(self):
        trend = self.bd.lines.trend[0]

        if trend == 1:
            self.log(f"Uptrend {self.data.datetime.date(0)} )")
        elif trend == -1:
            self.log(f"Downtrend {self.data.datetime.date(0)} )")
        # if trend == 1:
        #     print(f"{self.data.datetime.date(0)} → Uptrend")
        # elif trend == -1:
        #     print(f"{self.data.datetime.date(0)} → Downtrend")
        dt = self.data.datetime.datetime(0)
        self.results.append((dt, self.data.close[0], trend))

all_results_df, all_cerebros_objects, all_portfolio_histories = run_me(TestStrategy,400)
